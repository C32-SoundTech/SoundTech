...
class DiscussionGroupTemplate(AgentTemplate):
    ...
    def agents_run(self, participant_agents: dict, agent_input: dict, input_object: InputObject) -> dict:
        ...
        console = Console()
        table = Table(title=str(agent_input.get('input')))
        
        table.add_column("开始时间")
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        table.add_column(current_time)
        
        for i, participant_agent_name in enumerate(participant_agents.keys()):
            table.add_row(f"参与者{i + 1}", participant_agent_name)
        ...
        for i in range(total_round):
            with console.status(f"当前轮数[bold yellow]{i + 1}[/bold yellow]，总轮数[bold green]{total_round}[/bold green]", spinner="monkey") as status:
                for participant_agent_name, participant_agent in participant_agents.items():
                    ...
                    console.log(f"当前轮数[bold yellow]{i + 1}[/bold yellow]，总轮数[bold green]{total_round}[/bold green]，参与者：[bold blue]{participant_agent_name}[/bold blue]\n")
